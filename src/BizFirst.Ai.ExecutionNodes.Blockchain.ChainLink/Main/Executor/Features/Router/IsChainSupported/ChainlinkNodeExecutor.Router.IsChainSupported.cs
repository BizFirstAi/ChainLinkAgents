...
{
    private async Task<NodeExecutionResult> _Chainlink_Router_IsChainSupportedAsync(
        NodeExecutionContext nodeExecutionContext,
        CancellationToken cancellationToken = default)
    {
        //code-step: 1.1 - Validate settings exist and cast to RouterIsChainSupportedInfo
        if (mySettings?.ActiveInfo is not RouterIsChainSupportedInfo info)
        ...
        try
        {
            //code-step: 1.3 - Call Chainlink router service for the real on-chain isChainSupported read
            var r = await _routerService.IsChainSupportedAsync(info.NetworkName, info.DestinationChainSelector, cancellationToken);
            if (!r.Success)
            ...
            //code-step: 1.5 - Extract isChainSupported record from result
            var data = new Dictionary<string, object>
            {
                { "isChainSupported", r.IsChainSupported },
            };
            ...
            //code-step: 1.7 - Convert isChainSupported record to standard items array
        ...
        catch (Exception ex)
        {
            ...
            return resultManager.SetResultAsError(ExecutionConstants.OutputPorts.Error, $"router/isChainSupported failed: {ex.Message}", this);
        }
    }
}
