...
{
    private async Task<NodeExecutionResult> _Chainlink_Router_ConvertChainSelectorAsync(
        NodeExecutionContext nodeExecutionContext,
        CancellationToken cancellationToken = default)
    {
        //code-step: 1.1 - Validate settings exist and cast to RouterConvertChainSelectorInfo
        if (mySettings?.ActiveInfo is not RouterConvertChainSelectorInfo info)
        ...
        try
        {
            //code-step: 1.3 - Call Chainlink router service for the static chain-selector conversion lookup
            var r = _routerService.ConvertChainSelector(info.NetworkName, info.NativeChainID, info.ChainSelector);
            if (!r.Success)
            ...
            {
                { "network", r.NetworkName ?? string.Empty },
        ...
        catch (Exception ex)
        {
            ...
            return resultManager.SetResultAsError(ExecutionConstants.OutputPorts.Error, $"router/convertChainSelector failed: {ex.Message}", this);
        }
    }
}
